...
def generate_problem_with_unknowns(context: Context, standard: Standard) -> Expr:
    ...
    # print("first expr", first_expr)
    # print("first expr ans", first_expr_ans)
    count = 0
...
"""
given a word problem and a symbolic structure, generates a reasoning trace
"""
def generate_reasoning(problem, word_problem, final_answer):
    # reasoning_steps, final_answer = solve_with_steps(problem.variables, problem.question)
    # print(reasoning_steps, final_answer)
    client = OpenAI()
    print(problem.variables)
    match = re.match(r"^(\d+)", str(problem.standard))
    if match:
        grade = match.group(1)
    print(grade)
    messages = [{"role": "system", "content": "You are an teacher that generates reasoning to solve a math problem."}]
    messages.append({"role": "user", "content": f"The word problem is: {word_problem}. The final answer is {final_answer}. Generate a concise solution to the problem that gets us to the final answer. A student in grade {grade} should be able to understand your solution. Keep it short. Do not use LaTeX or Markdown in your solution."})
    response = client.chat.completions.create(
                    model="gpt-4o",
                    messages=messages
                    )
    gpt_reasoning = response.choices[0].message.content
    gpt_reasoning += f" Answer: {final_answer}"
    print(gpt_reasoning)
    return gpt_reasoning

"""
given a standard, num problems, list of themes, returns a json
containing worksheet data and a title for the worksheet
"""
def generate_json(standard: str, num_probs: int, themes: list):
    standards = load_standards()
    print("Standards implemented: ", standards.keys())
    standard = standards[standard]
    g_problems = generate_many(3*num_probs, standard, 0)
    data = []
    for i, g_problem in enumerate(g_problems):
        ...
        if themes == []:
            ...
            if standard.math_concept == "None":
                g_problem.theme = str(random.choice(themes))
            else:
                g_problem.theme = standard.math_concept
            ...
            if standard.id not in ["4.NF.A.2", "K.CC.C.7", "4.OA.B.4"]:
                if check_sym_struct(gpt_symbolic_structure, final_answer, standard):
                    p_seq_data.append({
                        ...
                        "reasoning": generate_reasoning(problem, word_problem, final_answer),
                        "answer": str(final_answer),
                        "tag": tag,
                        "theme": g_problem.theme
                        })
                else:
                    if j == 0:
                        break
